...
fn id(bytes: Vec<u8>) -> Result<Vec<u8>, String> {
    if bytes.len() != 16 {
        return Err(format!(
            "Invalid project metadata ID length {} (expected 16)",
            bytes.len()
        ));
    }
    Ok(bytes)
...
fn terms<T>(
    values: impl IntoIterator<Item = Result<Option<T>, String>>,
) -> Result<Merge<Option<T>>, String> {
    let values: Vec<_> = values.into_iter().collect::<Result<_, _>>()?;
    if values.len().is_multiple_of(2) {
        return Err(format!(
            "Invalid metadata merge term count {}",
            values.len()
        ));
    }
...
fn insert<K: Ord, V>(map: &mut BTreeMap<K, V>, key: K, value: V) -> Result<(), String> {
    if map.insert(key, value).is_some() {
        return Err("Duplicate project metadata key".into());
    }
    Ok(())
}
...
fn project_to_proto(value: &ProjectRecord) -> proto::ProjectRecord {
    proto::ProjectRecord {
        ...
        canonical_root: value.canonical_root.as_internal_file_string().to_owned(),
    }
}
fn project_from_proto(value: proto::ProjectRecord) -> Result<ProjectRecord, String> {
    let root =
        RepoPathBuf::from_internal_string(value.canonical_root).map_err(|err| err.to_string())?;
    if root.is_root() || !valid_label(&value.name) {
...
fn binding_to_proto(value: &BindingRecord) -> proto::BindingRecord {
    ...
    proto::BindingRecord {
        project_id,
        repository_view,
        connection_id: value.connection_id.to_bytes(),
        provider: "jjosh".into(),
        version: 1,
        representation,
        expression,
...
fn binding_from_proto(value: proto::BindingRecord) -> Result<BindingRecord, String> {
    if value.provider != "jjosh" || value.version != 1 {
        return Err(format!(
            "Unsupported representation provider/version {:?}/{}",
            value.provider, value.version
        ));
    }
    ...
    Ok(BindingRecord {
        target,
        connection_id: ConnectionId::new(id(value.connection_id)?),
        representation,
        base: value.base,
    })
}
fn observation_to_proto(value: &ConversionObservation) -> proto::ConversionObservation {
    proto::ConversionObservation {
        binding_id: value.binding_id.to_bytes(),
        binding: Some(binding_to_proto(&value.binding)),
        connection_id: value.connection_id.to_bytes(),
        ...
        terms: value
            .terms
            .iter()
            .map(|term| proto::ConversionTerm {
                canonical: term.canonical.as_ref().map(ObjectId::to_bytes),
                raw: term.raw.clone(),
            })
            .collect(),
        base: value.base.clone(),
    ...
    let terms = value
        ...
        .map(|term| {
            if (term.canonical.is_some() && term.raw.is_none())
                || term.canonical.as_ref().is_some_and(Vec::is_empty)
                || term.raw.as_ref().is_some_and(|raw| {
                    raw.is_empty()
            ...
            Ok(ConversionTerm {
                canonical: term.canonical.map(CommitId::new),
                raw: term.raw,
            })
        })
...
pub(crate) fn encode(view: &View) -> Option<proto::ProjectMetadata> {
    if view.project_state.is_empty()
        ...
        && view.observed_remote_connections.is_empty()
        && view.observed_remote_names.is_empty()
    {
        return None;
    }
    Some(proto::ProjectMetadata {
        version: if !view.observed_remote_connections.is_empty()
            || !view.observed_remote_names.is_empty()
        {
            3
        } else if view.project_state.remote_names.is_empty() {
            1
        } else {
            2
        },
        projects: view
            ...
            .projects
            .iter()
            .map(|(id, target)| proto::ProjectEntry {
                id: id.to_bytes(),
                terms: target
                    .iter()
                    .map(|value| proto::ProjectTerm {
                        value: value.as_ref().map(project_to_proto),
                    })
                    .collect(),
            })
            .collect(),
        bindings: view
            ...
            .bindings
            .iter()
            .map(|(id, target)| proto::BindingEntry {
                id: id.to_bytes(),
                terms: target
                    .iter()
                    .map(|value| proto::BindingTerm {
                        value: value.as_ref().map(binding_to_proto),
                    })
                    .collect(),
            })
            .collect(),
        labels: view
            ...
            .labels
            .iter()
            .map(|(label, target)| proto::LabelEntry {
                label: label.clone(),
                terms: target
                    .iter()
                    .map(|value| proto::IdTerm {
        ...
        connections: view
            .remote_connections
            .iter()
            .map(|(remote, target)| proto::ConnectionEntry {
        ...
        observations: view
            ...
            .iter()
            .map(|(key, target)| proto::ObservationEntry {
                remote: key.remote.as_str().to_owned(),
                name: key.name.as_str().to_owned(),
                kind: match key.kind {
                    ObservationKind::Bookmark => 1,
                    ObservationKind::Tag => 2,
                    ObservationKind::Revision => 3,
                },
                terms: target
                    .iter()
                    .map(|value| proto::ObservationTerm {
                        value: value.as_ref().map(observation_to_proto),
                    })
                    .collect(),
            })
            .collect(),
        remote_names: view
            ...
            .remote_names
            .iter()
            .map(|(connection, target)| proto::ScopedRemoteNameEntry {
                connection_id: connection.to_bytes(),
                terms: target
                    .iter()
                    .map(|value| proto::ScopedRemoteNameTerm {
                        value: value.as_ref().map(|identity| proto::ScopedRemoteName {
                            project_id: identity.project.to_bytes(),
...
pub(crate) fn decode(value: proto::ProjectMetadata, view: &mut View) -> Result<(), String> {
    ...
    if value.version < 3
        && (!value.observed_connections.is_empty() || !value.observed_remote_names.is_empty())
    {
        return Err("Historical remote identities require project metadata version 3".into());
    }
    for entry in value.observed_remote_names {
        ...
        insert(
            &mut view.observed_remote_names,
            ConnectionId::new(id(entry.connection_id)?),
            target,
        )?;
    }
    for entry in value.observed_connections {
        if entry.remote.is_empty() {
            return Err("Empty connection remote name".into());
        }
        let target = terms(entry.terms.into_iter().map(|term| {
            term.value
                .map(|bytes| id(bytes).map(ConnectionId::new))
                .transpose()
        }))?;
        insert(
            &mut view.observed_remote_connections,
            entry.remote.into(),
            target,
        )?;
    }
    ...
    for entry in value.bindings {
        let target = terms(
            entry
                .terms
                .into_iter()
                .map(|term| term.value.map(binding_from_proto).transpose()),
        )?;
        insert(
            &mut view.project_state.bindings,
            BindingId::new(id(entry.id)?),
            target,
        )?;
    }
    for entry in value.labels {
        if !valid_label(&entry.label) {
            return Err("Invalid project label".into());
        }
        let target = terms(entry.terms.into_iter().map(|term| {
            term.value
                .map(|bytes| id(bytes).map(ProjectId::new))
                .transpose()
        }))?;
    ...
    for entry in value.observations {
        if entry.remote.is_empty() || entry.name.is_empty() {
            return Err("Empty observation name".into());
        }
        let target = terms(
            entry
                .terms
                .into_iter()
                .map(|term| term.value.map(observation_from_proto).transpose()),
        )?;
...
pub(crate) fn validate(view: &View) -> Result<(), String> {
    let check_id = |id: &dyn ObjectId| {
        if id.as_bytes().len() == 16 {
            Ok(())
        } else {
            Err("Invalid project metadata ID length".to_owned())
        }
    };
    ...
    for (connection, target) in view
        .project_state
        .remote_names
        .iter()
        .chain(&view.observed_remote_names)
    {
        check_id(connection)?;
    ...
    for (remote, target) in view
        .remote_connections
        .iter()
        .chain(&view.observed_remote_connections)
    {
    ...
    for (key, target) in &view.project_observations {
        if key.remote.as_str().is_empty() || key.name.as_str().is_empty() {
            return Err("Empty observation name".into());
        }
        if key.kind == ObservationKind::Revision
            && (!key.name.as_str().len().is_multiple_of(2)
                || !key.name.as_str().bytes().all(|c| c.is_ascii_hexdigit()))
        {
        ...
        for observation in target.iter().flatten() {
            ...
            match key.kind {
                ObservationKind::Bookmark | ObservationKind::Tag
                    if !observation.raw_ref.starts_with("refs/") =>
                {
                    return Err("Reference observation requires a wire ref".into());
                }
                ObservationKind::Revision => {
                    ...
                    {
                        return Err(
                            "Revision observation requires its pinned raw OID and no wire ref"
                                .into(),
                        );
                    }
                }
...
fn check_binding<'a>(
    definitions: &mut BTreeMap<&'a BindingId, &'a BindingRecord>,
    id: &'a BindingId,
    record: &'a BindingRecord,
) -> Result<(), String> {
    if let Some(previous) = definitions.insert(id, record)
        && previous != record
    {
        return Err(format!(
            "Binding {id} has inconsistent immutable definitions"
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    fn view() -> View {
        ...
        view.project_state
            .bindings
            .insert(binding.clone(), Merge::normal(record.clone()));
        view.project_state
            .labels
            .insert("lib".into(), Merge::normal(project));
        view.remote_connections
            .insert("origin".into(), Merge::normal(connection.clone()));
        let observation = ConversionObservation {
            binding_id: binding,
            binding: record,
            connection_id: connection,
            endpoint: "file:///source".into(),
            raw_ref: "refs/heads/main".into(),
            terms: vec![
                ConversionTerm {
                    canonical: Some(CommitId::from_hex("11")),
                    raw: Some("aa".into()),
                },
                ConversionTerm {
                    canonical: Some(CommitId::from_hex("22")),
                    raw: Some("bb".into()),
                },
                ConversionTerm {
                    canonical: None,
        ...
        view.project_observations.insert(
            ...
            Merge::from_vec(vec![
                ...
                None,
                Some(ConversionObservation {
                    endpoint: "file:///other".into(),
                    ..observation
                }),
            ]),
        );
    ...
    fn evidence_roundtrip_retains_all_signed_witness_roots() {
        ...
        let roots: std::collections::HashSet<_> =
            wrapped.all_referenced_commit_ids().cloned().collect();
    ...
    fn rejects_mutated_binding_definition_in_historical_evidence() {
        ...
        let mut observation = source.project_observations[&key]
            .adds()
            .flatten()
            .next()
            .unwrap()
        ...
        source
            .project_observations
            .insert(key, Merge::normal(observation));
    ...
    fn revision_witness_is_persisted_without_a_remote_reference() {
        ...
        source
            .project_observations
            .insert(key.clone(), Merge::normal(observation));
        let mut restored = View::make_root(CommitId::from_hex("00"));
        decode(encode(&source).unwrap(), &mut restored).unwrap();
    ...
    fn scoped_names_roundtrip_conflicts_and_reject_version_downgrade() {
        ...
        // an empty extension would change every historical operation identity.
        let legacy_state = (
            ("projects".to_owned(), source.project_state.projects.clone()),
        ...
        let mut restored = View::make_root(CommitId::from_hex("00"));
        decode(encoded.clone(), &mut restored).unwrap();
        assert_eq!(restored, source);
        assert_ne!(crate::content_hash::blake2b_hash(&restored), old_hash);
        ...
        assert!(decode(malformed, &mut View::make_root(CommitId::from_hex("00"))).is_err());
    }

    #[test]
    fn historical_identity_roundtrip_preserves_signed_terms_without_definitions() {
        let mut source = view();
        let connection = source
            .remote_connections
            .values()
            ...
            .clone();
        let project = source.project_state.projects.keys().next().unwrap().clone();
        source.observed_remote_connections = std::mem::take(&mut source.remote_connections);
        source.observed_remote_names.insert(
            connection,
            Merge::from_vec(vec![
                Some(ScopedRemoteName {
                    project: project.clone(),
        ...
        source.project_state = ProjectState::default();
        let encoded = encode(&source).unwrap();
        let mut restored = View::make_root(CommitId::from_hex("00"));
        decode(encoded.clone(), &mut restored).unwrap();
        assert_eq!(source, restored);
        assert_eq!(
            crate::content_hash::blake2b_hash(&source),
            crate::content_hash::blake2b_hash(&restored)
        );
    ...
    #[test]
    fn disconnected_restored_evidence_does_not_reactivate_connections() {
        let mut source = view();
        source.remote_connections.clear();
        source.project_state.bindings.clear();
        source.project_state.projects.clear();
        source.project_state.labels.clear();
        let mut restored = View::make_root(CommitId::from_hex("00"));
        decode(encode(&source).unwrap(), &mut restored).unwrap();
        let wrapped = crate::view::View::new(restored, true);
        for key in source.project_observations.keys() {
            assert!(wrapped.validate_project_observation(key).is_ok());
        }
        assert!(wrapped.store_view().remote_connections.is_empty());
        assert!(wrapped.project_state().bindings.is_empty());
    }
}
