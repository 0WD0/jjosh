...
pub async fn cmd_git_remote_add(
    ...
    let preparation_remote = match &requested_scope {
        Some(project) => {
            super::scoped_remote_name(workspace_command.repo().view(), &local_name, project)?
        }
    ...
    tx.repo_mut().view_mut().clear_remote_observations(&remote);
