...
pub async fn cmd_git_remote_remove(
    ...
    let identity = view.remote_identity(&remote).map_err(crate::command_error::user_error)?;
    ...
    options.repo_config = remove_remote_from_repo_config(ui, command.raw_config(), view, local_name, labels.as_deref())?;
    ...
    let connection = git::remote_connection_id(&git_repo, &remote).map_err(crate::command_error::user_error)?;
    ...
    if let Some(connection) = connection {
        tx.repo_mut().view_mut().project_state_mut().remote_names.remove(&connection);
    ...
    view.project_observations.retain(|key, _| key.remote != remote);
