...
fn test_git_remote_with_slashes() {
    ...
    // The remote can be renamed.
    work_dir
        .run_jj(["git", "remote", "rename", "slash/origin", "origin"])
        .success();
    let output = work_dir.run_jj(["git", "remote", "list"]);
    insta::assert_snapshot!(output, @"
...
fn test_disconnected_managed_remote_local_lifecycle_without_provider() -> TestResult {
    ...
    let connection = git_repo
        .config_snapshot()
        .string("remote.origin.jjosh-connectionId")
        .unwrap()
        .to_string();
    ...
    work_dir
        .run_jj(["git", "remote", "rename", "origin", "renamed"])
        .success();
    work_dir
        .run_jj([
            "git",
    ...
    let config = git_repo.config_snapshot();
    assert_eq!(
        config.string("remote.renamed.url").unwrap().to_string(),
        "https://example.com/reconnected"
    );
    assert_eq!(
        config
            .string("remote.renamed.jjosh-connectionId")
            .unwrap()
            .to_string(),
        connection
    );
    assert_eq!(
        config
            .string("remote.renamed.jjosh-requiredCapability")
            .unwrap()
            .to_string(),
        "jjosh-v1"
    );
    work_dir
        .run_jj(["git", "remote", "remove", "renamed"])
        .success();
