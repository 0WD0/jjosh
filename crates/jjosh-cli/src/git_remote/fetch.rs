...
fn source_ref(reference: &str) -> Option<(GitRefKind, &str)> {
    reference.strip_prefix("refs/heads/").map(|name| (GitRefKind::Bookmark, name))
        .or_else(|| reference.strip_prefix("refs/tags/").map(|name| (GitRefKind::Tag, name)))
}
...
fn canonical_ref(session: &Session, kind: GitRefKind, source: &str) -> String {
    ...
    format!("{prefix}{}/{}", session.name.as_str(), session.local_name(source).as_str())
}
...
impl Converted {
    fn absent() -> Self {
        Self { target: RefTarget::absent(), mirror: None }
    }

    fn native(target: RefTarget) -> Result<Self, CommandError> {
        let mirror = target.as_normal().map(|id| gix::ObjectId::try_from(id.as_bytes()))
...
fn refs_prefixed(
    ...
    for reference in platform.prefixed(prefix).map_err(user_error)? {
        ...
        let Ok(name) = std::str::from_utf8(reference.name().as_bstr()) else { continue };
...
fn install_refs(
    ...
    let deletions = old.iter().filter(|(name, _)| !new.contains_key(*name))
        .map(|(name, id)| Ok(RefEdit {
    ...
    let updates = new.iter().map(|(name, id)| Ok(RefEdit {
        name: name.as_str().try_into().map_err(user_error)?,
        deref: false,
...
pub(super) async fn run(
    ...
    let endpoint = session.endpoint_url(&git, gix::remote::Direction::Fetch).map_err(user_error)?;
    ...
    let transaction = if session.project.is_some() || session.josh.is_some() {
        Some(crate::interop::open_josh_transaction(&session.git_path, false)?)
    } else {
        None
    };
    ...
    for (kind, prefix) in [
        ...
        for (name, id) in refs_prefixed(&git, &prefix)? {
                ...
                && selected(kind, source)
            {
                ...
                let source_prefix = if kind == GitRefKind::Bookmark { "refs/heads/" } else { "refs/tags/" };
        ...
        for name in repo.view().git_refs().keys() {
            ...
            {
                let source_prefix = if kind == GitRefKind::Bookmark { "refs/heads/" } else { "refs/tags/" };
                converted.entry(format!("{source_prefix}{source}")).or_insert_with(Converted::absent);
            }
        }
    }
    if let Some(view) = repo.view().get_remote_view(&session.name) {
        for (kind, refs) in [(GitRefKind::Bookmark, &view.bookmarks), (GitRefKind::Tag, &view.tags)] {
            for name in refs.keys() {
                if let Some(source) = session.source_name(name)
                    && selected(kind, source)
                {
                    let prefix = if kind == GitRefKind::Bookmark { "refs/heads/" } else { "refs/tags/" };
                    converted.insert(format!("{prefix}{source}"), Converted::absent());
                }
            }
        }
    }
    for (kind, expression) in [(GitRefKind::Bookmark, &selection.bookmark), (GitRefKind::Tag, &selection.tag)] {
        for name in expression.exact_strings().filter(|name| selected(kind, name)) {
            let prefix = if kind == GitRefKind::Bookmark { "refs/heads/" } else { "refs/tags/" };
    ...
    let native_path = if session.project.as_ref().is_some_and(|project| project.native)
    ...
    if let Some(path) = native_path {
        let transaction = transaction.as_ref().expect("native project has a Josh transaction");
        ...
        if std::fs::canonicalize(source_git.git_repo_path())? == std::fs::canonicalize(&session.git_path)? {
            return Err(user_error("Native fetch requires an external source repository"));
        }
        let source = crate::native_source::NativeSource::read_selected(workspace.repo_loader(), &selection)
            .await.map_err(user_error)?;
        let project = session.project.as_ref().expect("native project checked above");
        let known = crate::native_project::anchors(repo, &transaction, &project.name).await.map_err(user_error)?;
        let imported = crate::native_import::import_source(&source, repo, &project.name, &project.mount, known)
            .await.map_err(user_error)?;
        for (prefix, refs) in [("refs/heads/", &source.view.local_bookmarks), ("refs/tags/", &source.view.local_tags)] {
            for (name, target) in refs {
                let target = RefTarget::from_merge(target.as_merge().map(|term| {
                    term.as_ref().map(|id| imported.ids[id].clone())
                }));
                converted.insert(format!("{prefix}{}", name.as_str()), Converted::native(target)?);
            }
        }
        crate::native_project::record_imported_boundaries(
            &transaction, &project.name, &imported,
            source.view.local_bookmarks.values().chain(source.view.local_tags.values())
        ...
        for (name, id) in refs_prefixed(&git, &raw_prefix)? {
            ...
            if source_ref(source).is_some_and(|(kind, name)| selected(kind, name)) {
                ...
                converted.entry(source.to_owned()).or_insert_with(Converted::absent);
            }
        }
        let outcome = crate::git_transport::fetch::fetch(
            session.remote(&git, gix::remote::Direction::Fetch).map_err(user_error)?,
            |reference| {
                std::str::from_utf8(reference.unpack().0).ok()
                    .and_then(source_ref).is_some_and(|(kind, name)| selected(kind, name))
            },
        ...
        for reference in &outcome.advertised {
            ...
            if source_ref(name).is_some_and(|(kind, name)| selected(kind, name)) {
                advertised.insert(name.to_owned(), id.filter(|id| !id.is_null()).map(ToOwned::to_owned));
                converted.entry(name.to_owned()).or_insert_with(Converted::absent);
            }
        }
        ...
        for reference in &outcome.received {
            ...
            let id = id.ok_or_else(|| user_error("Received reference has no object ID"))?.to_owned();
        ...
        if let Some(transaction) = &transaction {
            for source in converted.keys().filter(|name| name.starts_with("refs/heads/")) {
                match advertised.get(source).copied().flatten() {
                    Some(id) => crate::link_refs::record_observation(transaction, &endpoint, source, id)?,
        ...
        for (name, direct) in &received {
            let object = git.find_object(*direct).map_err(user_error)?
                .peel_tags_to_end().map_err(user_error)?;
        ...
        if let Some(project) = session.project.as_ref().filter(|project| project.native) {
            let transaction = transaction.as_ref().expect("native project has a Josh transaction");
            let known = crate::native_project::anchors(repo, &transaction, &project.name).await.map_err(user_error)?;
            let ids: Vec<_> = commits.values().map(|id| CommitId::from_bytes(id.as_bytes())).collect();
            ...
            let source = crate::native_source::NativeSource::read_view(
                repo.store().clone(), repo.op_store().clone(), view, String::new(),
            ).await.map_err(user_error)?;
            let imported = crate::native_import::import_source(&source, repo, &project.name, &project.mount, known)
                .await.map_err(user_error)?;
            for (name, raw) in &commits {
                let id = &imported.ids[&CommitId::from_bytes(raw.as_bytes())];
                converted.insert(name.clone(), Converted::native(RefTarget::normal(id.clone()))?);
            }
            crate::native_project::record_imported_boundaries(&transaction, &project.name, &imported, &ids)
                .map_err(user_error)?;
        } else if let Some(filter) = session.filter() {
            let transaction = transaction.as_ref().expect("projection has a Josh transaction");
            ...
            for (name, raw) in &commits {
                let filtered = josh_core::filter_commit(&transaction, filter.clone(), *raw).map_err(user_error)?;
                if filtered.is_null() { continue; }
                let canonical = if let Some(project) = &session.project {
                    crate::link_fetch::canonicalize_filtered_graph(
                        repo, &transaction, Path::new(project.mount.as_internal_file_string()), filtered, &mut matches,
                    ).await?
                } else {
                    CommitId::from_bytes(filtered.as_bytes())
                };
                converted.insert(name.clone(), Converted::native(RefTarget::normal(canonical))?);
            }
        } else {
            for (name, commit) in &commits {
                converted.insert(name.clone(), Converted {
    ...
    for (source, converted) in converted {
        ...
        let canonical_git_oid = converted.target.as_normal()
            .map(|id| gix::ObjectId::try_from(id.as_bytes())).transpose().map_err(user_error)?;
        observations.push(GitRemoteObservation {
            kind,
            symbol: RemoteRefSymbolBuf { name: session.local_name(name), remote: session.name.clone() },
