...
impl Session {
    fn open(
        ...
        if let Some(id) = &connection {
            for other in git.remote_names() {
                ...
                if other != name.as_str()
                    && config_string(git, &format!("remote.{other}.jjosh-connectionId"))
                        .map_err(user_error)?
                        .as_deref()
                        == Some(id.hex().as_str())
                {
                    return Err(user_error(
                        "Multiple remote aliases claim the same connection identity",
                    ));
                }
            }
        }
        let binding = connection
            .as_ref()
            .map(|id| state.binding_for_connection(id).map_err(user_error))
            .transpose()?
            .flatten()
            .map(|(id, record)| (id, record.clone()));
        let required = config_string(
            git,
            &format!("remote.{}.jjosh-requiredCapability", name.as_str()),
        )
        .map_err(user_error)?;
        if required
            .as_deref()
            .is_some_and(|capability| capability != "jjosh-v1")
        {
            return Err(user_error("Unknown required remote capability"));
        }
        if required.is_some() && binding.is_none() {
            return Err(user_error(
                "Managed connection has no active binding in this operation",
            ));
        }
        if binding.is_none()
        ...
        if let Some((_, record)) = &binding {
            if git.object_hash() != gix::hash::Kind::Sha1 {
                return Err(user_error("Project conversion requires SHA-1"));
            }
            if let BindingTarget::Project(id) = &record.target {
                ...
                let definition = state
                    ...
                    .ok_or_else(|| user_error("Project definition is unavailable"))?;
                let labels: Vec<_> = state
                    .labels
                    .iter()
                    .filter_map(|(label, value)| {
                        (value.as_resolved().and_then(Option::as_ref) == Some(id)).then_some(label)
                    })
                    .collect();
                ...
                project = Some(Project {
                    ...
                    mount: definition.canonical_root.clone(),
                    label: (*label).clone(),
                });
            }
            filter = if record.representation == Representation::Whole {
                None
            } else {
                Some(crate::binding_config::filter(&record.representation).map_err(user_error)?)
            };
            if let (Some(value), Some(project)) = (filter, &project) {
                filter = Some(value.prefix(project.mount.as_internal_file_string()));
            }
        }
        Ok(Self {
            name: name.to_owned(),
    ...
    fn push_scope(
        ...
        let selected = if let Some(source) = source {
            ...
            if selected.binding.is_none() {
                return Err(user_error(
                    "--source must select an active conversion binding",
                ));
            }
            if self.binding.is_some()
                && self.binding.as_ref().map(|(id, _)| id)
                    != selected.binding.as_ref().map(|(id, _)| id)
            {
                return Err(user_error(
                    "Destination has its own immutable binding; --source cannot replace its representation",
                ));
            }
            selected
        } else {
        ...
        if selected.project.as_ref().map(|value| &value.id) != project {
            return Err(user_error(
                "Selected reference project does not match the destination/source binding",
            ));
        }
        if project.is_some() && selected.binding.is_none() {
            return Err(user_error(
                "Project publication requires a destination binding or explicit --source",
            ));
        }
        Ok(selected)
    }
    ...
    pub fn whole(&self) -> bool {
        self.binding
            .as_ref()
            .is_some_and(|(_, record)| record.representation == Representation::Whole)
    }
    ...
    async fn anchors(
        ...
        if let Some(project) = &self.project {
            for (id, value) in &self.state.bindings {
                let Some(record) = value.as_resolved().and_then(Option::as_ref) else {
                    continue;
                };
                if id == self.binding_id()
                    || record.target != BindingTarget::Project(project.id.clone())
                    || record.representation != Representation::Whole
                {
                    continue;
                }
                if !crate::native_project::is_offline_binding(transaction, id)
                    .map_err(user_error)?
                {
                    continue;
                }
                for (raw, canonical) in crate::native_project::anchors(repo, transaction, id)
                    .await
                    .map_err(user_error)?
                {
                    if known
                    ...
                    {
                        return Err(user_error("Ambiguous offline native correspondence"));
                    }
...
impl GitRemoteSession for Session {
    fn local_name(&self, source: &str) -> RefNameBuf {
        self.project.as_ref().map_or_else(
            || source.into(),
            |project| crate::ref_names::local_name(&project.label, source).into(),
        )
    }
    fn source_name<'a>(&self, local: &'a RefName) -> Option<&'a str> {
        match &self.project {
            Some(project) => crate::ref_names::unscoped_name(&project.label, local.as_str()),
