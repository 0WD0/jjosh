...
fn link_push_prunes_empty_branches_without_losing_meaningful_merges() {
    ...
    jjosh(
        &client,
        &["new", "deps/main@deps-upstream", "-m", "Right change"],
    );
    ...
    jjosh(
        &client,
        &["new", "deps/main@deps-upstream", "-m", "Empty side branch"],
    );
...
fn malformed_link_metadata_is_not_silently_omitted() {
    ...
    jjosh(&client, &["new", "@", "deps/main@deps-upstream"]);
    assert_eq!(
...
fn selected_link_fetch_preserves_work_and_handles_tags_rewrites_and_deletions() {
    for colocated in [false, true] {
        ...
        let local = commit_id(&client, "@");
        let initial = commit_id(&client, "deps/main@deps-upstream");
        ...
        assert_eq!(commit_id(&client, "deps/main@deps-upstream"), initial);
        assert_eq!(
            file_at_revision(
                &client,
                "remote_tags(exact:\"deps/v1\", exact:\"deps-upstream\")",
                "deps/value.txt"
            ),
            b"selection-v1\n"
        );
        ...
        let updated = commit_id(&client, "deps/main@deps-upstream");
        ...
        assert_eq!(
            file_at_revision(&client, "deps/main@deps-upstream", "deps/value.txt"),
            b"upstream v2\n"
        );
        ...
        assert_eq!(commit_id(&client, "deps/main@deps-upstream"), updated);
        ...
        assert_eq!(commit_id(&client, "deps/keep@deps-upstream"), initial);
        assert_eq!(
            file_at_revision(
                &client,
                "remote_tags(exact:\"deps/v1\", exact:\"deps-upstream\")",
        ...
        assert_eq!(
            commit_id(
                &client,
                "remote_bookmarks(exact:\"deps/keep\", exact:\"deps-upstream\")"
            ),
            ""
        );
        assert_eq!(
...
fn imported_soft_fork_attaches_upstream_without_reimporting_local_history() {
    ...
    jjosh(&client, &["new", "@", "app/workspace/default"]);
    let fork = commit_id(&client, "app/main");
    let before = jjosh(
        &client,
        &[
            "log",
    ...
    assert_eq!(commit_id(&client, "app/main"), fork);
    assert_eq!(
        jjosh(
            &client,
            &[
    ...
    assert_eq!(commit_id(&client, "app/main"), fork);
    assert_eq!(
        commit_id(&client, "parents(app/main@app-upstream)"),
        commit_id(&client, "parents(app/main)")
    );
    jjosh(&client, &["new", "@", "app/main@app-upstream"]);
    assert_eq!(
